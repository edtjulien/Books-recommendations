...
class BookReco:
    ...
    def fit(self, data):
        ...
        if self.df_users is not None:
            self.users_cs = self.__get_cosine_similarity(
                prefix=None, data=self.df_users
            )
    ...
    def predict(self, book_id):
        ...
        try:
            index_book = self.data_empty.query(
                "book_id == @book_id"
        ...
        for vector in self.vectors:
            ...
            self.data_score = pd.concat(
                [self.data_score, pd.Series(score * weight, name=vector["col_name"])],
                axis=1,
            )
        ...
        for scalar in self.scalars:
            ...
            self.data_score = pd.concat(
                [
                    self.data_score,
                    pd.Series(scalar["scaled"] * weight, name=scalar["col_name"]),
                ],
                axis=1,
            )
        ...
        if self.df_jaccard is not None:
            ...
            if score is not None:
                ...
                self.data_score = self.data_score.merge(
                    pd.Series(score, name="jaccard"),
                    how="left",
                    left_index=True,
                    right_index=True,
                )
        ...
        if self.users_cs is not None:
            ...
            if score is not None:
                ...
                self.data_score = pd.concat(
                    [self.data_score, pd.Series(score, name="users")], axis=1
                )
        ...
        return pd.Series(
            self.data_score[self.data_score.index != book_id], name="score"
        )
    ...
    def format_tojson(self, scores, data, max_books=5):
        data = data.copy().merge(
            scores, how="left", left_on="book_id", right_index=True
        )
